...
impl Ask {
    pub async fn create<C: AsRef<str>, D: AsRef<str>>(pool: &DbPool, askee: i32, content: C, dedup: D) -> Result<i32, sqlx::Error> {
        let id: i32 = sqlx::query("insert into ask (askee, content, dedup) values ($1, $2, $3) returning id")
            .bind(askee)
            .bind(content.as_ref())
            .bind(dedup.as_ref())
            .fetch_one(pool)
...
mod timestamp_option {
            ...
            D: Deserializer<'de>,
    {
        ...
        let result = match formatted {
            None => None,
            Some(formatted) => Some(
                NaiveDateTime::parse_from_str(&*formatted, FORMAT).map_err(D::Error::custom)?
            )
        };
        Ok(result)
    }
